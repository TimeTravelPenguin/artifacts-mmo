...
pub trait EventsApi: Send + Sync {
    ...
    async fn get_all_active_events_events_active_get<'page, 'size>(
        &self,
        page: Option<i32>,
        size: Option<i32>,
    ...
    async fn get_all_events_events_get<'page, 'size>(
        &self,
        page: Option<i32>,
        size: Option<i32>,
    ) -> Result<models::DataPageEventSchema, Error<GetAllEventsEventsGetError>>;
}
    ...
    ) -> Result<models::DataPageActiveEventSchema, Error<GetAllActiveEventsEventsActiveGetError>>
    {
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            match local_var_content_type {
                ...
                ContentType::Unsupported(local_var_unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{local_var_unknown_type}` content type response that cannot be converted to `models::DataPageActiveEventSchema`")))),
            }
        } else {
    ...
    async fn get_all_events_events_get<'page, 'size>(
        ...
        if !local_var_status.is_client_error() && !local_var_status.is_server_error() {
            match local_var_content_type {
                ...
                ContentType::Unsupported(local_var_unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{local_var_unknown_type}` content type response that cannot be converted to `models::DataPageEventSchema`")))),
            }
        } else {
